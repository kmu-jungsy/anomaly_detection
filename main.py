...
def parsing_args(c):
    ...
    parser.add_argument('--posco-apply-train-mask', action='store_true', default=False,
                        help='for POSCO train images: apply <folder>_mask.jpg so black mask area becomes black and white area keeps original')
    parser.add_argument('--posco-mask-dir', default='./mask', type=str,
    ...
    if c.dataset == 'mvtec':
        ...
        from datasets import POSCO_CLASS_NAMES
        setattr(c, 'data_path', './data/posco')
        if c.class_names == ['all']:
            setattr(c, 'class_names', POSCO_CLASS_NAMES)

    if getattr(args, 'data_path', ''):
        setattr(c, 'data_path', args.data_path)

    if getattr(c, 'dataset', '') == 'mvtec' and getattr(c, 'class_name', '') == 'transistor':
